...
namespace Geexu\Blog\Model\Import;
...
use Geexu\Blog\Model\Config\Source\AuthorType;
use Geexu\Blog\Model\Config\Source\Comments\Status;
...
/**
 * Class MageFanM2
 * @package Geexu\Blog\Model\Import
 */
class MageFanM2 extends AbstractImport
{
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
             ...
             */
            $topicPostTable = $this->_resourceConnection->getTableName('geexu_blog_post_topic');
        ...
        $this->_registry->register('geexu_import_post_ids_collection', $oldPostIds);
        $this->_registry->register('geexu_import_post_statistic', $statistics);
    ...
    protected function _importTags($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            $tagPostTable = $this->_resourceConnection->getTableName('geexu_blog_post_tag');
            $sqlTagPost   = "SELECT * FROM `" . $data['table_prefix'] . self::TABLE_POST_TAG . "` ";
            $result       = mysqli_query($connection, $sqlTagPost);
            $oldPostIds   = $this->_registry->registry('geexu_import_post_ids_collection');
        ...
        $this->_registry->register('geexu_import_tag_statistic', $statistics);
    }
    ...
    protected function _importCategories($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
             ...
             */
            $categoryPostTable = $this->_resourceConnection->getTableName('geexu_blog_post_category');
        ...
        $this->_registry->register('geexu_import_category_statistic', $statistics);
    }
    ...
    protected function _importComments($data, $connection)
    {
                ...
                if ($comment['is_imported']) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update($this->_resourceConnection
                            ->getTableName('geexu_blog_comment'), [
        ...
        $this->_registry->register('geexu_import_comment_statistic', $statistics);
    }
    ...
    protected function _importAuthors($data, $connection)
    {
        ...
        foreach ($updateData as $postId => $authorId) {
            ...
            $this->_resourceConnection->getConnection()
                ->update(
                    $this->_resourceConnection->getTableName('geexu_blog_post'),
                    ['author_id' => $authorId],
                    $where
                );
        }
        $statistics = $this->_getStatistics('authors', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('geexu_import_user_statistic', $statistics);
    }
    ...
    private function _updatePosts($post, $where)
    {
        ...
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('geexu_blog_post_category'), $where);
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('geexu_blog_post_tag'), $where);
    }
    ...
    private function _updateCategories($category, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('geexu_blog_category'), [
                'name'             => $category['name'],
                'url_key'          => $category['url_key'],
                'meta_robots'      => $category['meta_robots'],
                'store_ids'        => $category['store_ids'],
