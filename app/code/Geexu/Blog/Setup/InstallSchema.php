        ...
        if (!$this->helperData->versionCompare('2.3.0')):
            if (!$installer->tableExists('geexu_blog_post')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_post'))
            ...
            if (!$installer->tableExists('geexu_blog_tag')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_tag'))
            ...
            if (!$installer->tableExists('geexu_blog_topic')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_topic'))
            ...
            if (!$installer->tableExists('geexu_blog_category')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_category'))
            ...
            if (!$installer->tableExists('geexu_blog_post_tag')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_post_tag'))
                    ->addColumn('tag_id', Table::TYPE_INTEGER, null, [
                        'unsigned' => true,
                        'primary' => true,
                        'nullable' => false
                    ...
                    ->addIndex($installer->getIdxName('geexu_blog_post_tag', ['post_id']), ['post_id'])
                    ->addIndex($installer->getIdxName('geexu_blog_post_tag', ['tag_id']), ['tag_id'])
                    ->addForeignKey(
                        $installer->getFkName('geexu_blog_post_tag', 'post_id', 'geexu_blog_post', 'post_id'),
                        'post_id',
                    ...
                    ->addForeignKey(
                        $installer->getFkName('geexu_blog_post_tag', 'tag_id', 'geexu_blog_tag', 'tag_id'),
                        'tag_id',
            ...
            if (!$installer->tableExists('geexu_blog_post_topic')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_post_topic'))
                    ->addColumn('topic_id', Table::TYPE_INTEGER, null, [
                        'unsigned' => true,
                        'primary' => true,
                        'nullable' => false
                    ...
                    ->addIndex($installer->getIdxName('geexu_blog_post_topic', ['post_id']), ['post_id'])
                    ->addIndex($installer->getIdxName('geexu_blog_post_topic', ['topic_id']), ['topic_id'])
                    ->addForeignKey(
                        $installer->getFkName('geexu_blog_post_topic', 'post_id', 'geexu_blog_post', 'post_id'),
                        'post_id',
                        $installer->getTable('geexu_blog_post'),
                        'post_id',
                        Table::ACTION_CASCADE
            ...
            if (!$installer->tableExists('geexu_blog_post_category')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('geexu_blog_post_category'))
                    ->addColumn('category_id', Table::TYPE_INTEGER, null, [
                        'unsigned' => true,
                        'primary' => true,
                        'nullable' => false
                    ...
                    ->addIndex($installer->getIdxName('geexu_blog_post_category', ['category_id']), ['category_id'])
                    ->addIndex($installer->getIdxName('geexu_blog_post_category', ['post_id']), ['post_id'])
                    ->addForeignKey(
                        $installer->getFkName(
                            'geexu_blog_post_category',
                            'category_id',
                            'geexu_blog_category',
                            'category_id'
                        ),
